function boot() {
 async function client() {
  const HAS_GPU = "gpu" in navigator,
   ...
   gpu = HAS_GPU ? await navigator.gpu.requestAdapter().then(adapter => adapter.requestDevice()) : null,
   style = document.querySelector("head>style"),
   { body } = document,
   engine = {
    create() {
     if (!HAS_GPU) throw (body.innerHTML = "WebGPU is not supported.")
     const Utils = {
       foundation: classObj => {
 ...
 * {
 ...
 cursor: pointer;
   ...
   explorer = {
    /* TODO: Add an abstraction layer between this virtual object and setting the document?
      documentAbstraction.set("style.css", `html, body {...}`)
     */
    create(filename) {
     ...
     /* TODO: Add an abstraction layer between this virtual object and setting globalThis.hash?
      hashAbstraction.set("explorer/filename.txt", `html, body {...}`)
     */
     const onclickAttr = `event.stopPropagation();globalThis.hash='#'+this.getAttribute('data-hash')`,
      selectAttr = ` data-selected="true"`,
      directory = { "Empty Document": "0", ".htaccess": "1", "dev.server.js": "2", "index.php": "3", "server.js": "4", "3D Preview": "6" },
   ...
   loop = now => {
    averageFrameTime += (now - sessionTime - averageFrameTime) / 20
    sessionTime = now
    const framerate = Math.round(1000 / averageFrameTime)

    if (now - throttleStartTime >= throttleDuration && location.hash !== hash) {
    ...
    if (document.hash !== hash) {
     ...
     TransitionSpace[document.hash + hash]()
     document.hash = hash
    }
  ...
  var sessionTime = performance.now(),
  ...
  onhashchange = () => {
   ...
   if (!location.hash) history.replaceState({}, null, HOME_HASH)
   hash = location.hash
   loop((throttleStartTime = performance.now()))
  }
  ...
  onhashchange()
  TESTING_ABCDEFGHIJKLMNOPQRSTUVWXYZ: {
   class Part {
    indexOf(data) {
     let index = -1n
     if (this.terminal) {
      if (this.range) {
       if (typeof data === "object" && this.name in data && typeof data[this.name] === "bigint" && data[this.name] >= 0n && data[this.name] < this.cardinality)
        index = data[this.name]
    ...
    get(index) {
     if (index < 0n || index >= this.cardinality) throw new RangeError(`index ${index} out of range in ${this.name} (cardinality ${this.cardinality})`)
     let data = {}
     if (this.terminal) data[this.name] = this.range ? index : 0n
     else if (this.composite) {
      const subdata = (data[this.name] = {})
      for (let e = 0; e < this.length; e++) {
       const unit = this.parts.slice(e + 1).reduce((u, part) => u * part.cardinality, 1n),
        part = this.parts[e]
       Object.assign(subdata, part.get(index / unit))
       index %= unit
      }
     } else {
      let offset = 0n
     ...
     if (this.terminal) {
      ...
      if (composite) {
       ...
       this.parts = parts.map(e => {
        ...
        if (dimensions.has(part.name)) throw `duplicate identifier "${part.name}" in composite part ${this.name}.`
        dimensions.add(part.name)
        return part
       })
       this.cardinality = this.parts.reduce((cardinality, part) => cardinality * part.cardinality, BigInt(composite))
      } else {
       let offset = 0n
       this.planes = {}
   ...
   const Core = new Part({
    ...
    parts: [
     "empty",
     new Part({ name: "explorer", parts: [new Part({ name: "filename", parts: [".htaccess", "dev.server.js", "server.js", "index.php", "."] })], composite: true }),
     new Part({ name: "engine", parts: [new Part({ name: "byte", bitdepth: 8n })], composite: true }),
    ],
   })

   console.log("TESTING_ABCDEFGHIJKLMNOPQRSTUVWXYZ", Core.cardinality)
  }
 }
