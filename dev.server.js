 ...
 static Composite = class Composite extends this {
  constructor(name, factors) {
   ...
   this.parts = factors.reduceRight((parts, part) => {
    if (typeof part === "string") part = new Parts(part)
    if (!(part instanceof Parts)) throw new TypeError(`unexpected ${typeof part} encountered as factor of composite ${this.name}`)
  ...
  populate(index) {
   ...
   //console.group("Populating", `${oldindex} => ${index}`, this)
  ...
  leave() {
   //console.group("Leaving.\nInspect for memory leakes.", this)
   super.leave()
 ...
 static Decision = class Decision extends this {
  ...
  constructor(name, choices) {
   ...
   this.choices = choices.map((e, i) => {
    const choice = typeof e === "string" ? new Parts(e) : e
    if (!(choice instanceof Parts)) throw new TypeError(`unexpected ${typeof e} encountered as choice ${i} of decision ${this.name}`)
...
class Client extends Parts.Decision {
 static Explorer = class Explorer extends Parts.Decision {
  static Selection = class Selection extends Parts.Composite {
   static Anchor = class Anchor extends Parts.Decision {
    constructor() {
     super("anchor", Client.Explorer.items)
    }
    enter() {
     //console.group("Entering", this)
     super.enter()
     this.itemNodes = this.controller.itemNodes
     this.defaultNode = this.itemNodes[Client.Explorer.items[0]]
    ...
    populate(index) {
     ...
     this.anchorNode = this.itemNodes[Client.Explorer.items[index]]
   ...
   static Mask = class Mask extends Parts.Bitmask {
    sparseArray = {}
    constructor() {
     super("mask", Client.Explorer.items.length - 1)
    }
    enter() {
     //console.group("Entering", this)
     super.enter()
    ...
    populate(index) {
     ...
     const prefix = [...index.toString(2).padStart(Client.Explorer.items.length - 1, "0")].map(c => c === "1").reverse(),
     ...
     for (let i = 0; i < mask.length; i++) {
      if (i in this.sparseArray) {
       ...
       this.sparseArray[i] = this.itemNodes[Client.Explorer.items[i]]
   ...
   constructor() {
    super("selection", [new Client.Explorer.Selection.Anchor(), new Client.Explorer.Selection.Mask()])
   }
   enter() {
    //console.group("Entering", this)
    this.itemNodes = this.controller.itemNodes
  ...
  static items = [
   ...
   "bitmask.core.parts",
   "explorer.client.core.parts",
   "selection.explorer.client.core.parts",
   "anchor.selection.explorer.client.core.parts",
   "mask.selection.explorer.client.core.parts",
   "engine.client.core.parts",
  ]
  constructor() {
   super("explorer", ["no-selection", new Client.Explorer.Selection()])
  ...
  enter() {
   ...
   this.nav = document.body.appendChild(document.createElement("nav"))
   this.main = document.body.appendChild(document.createElement("main"))
   const notepad = this.main.appendChild(document.createElement("pre"))
   notepad.textContent = `Hello, world.`
   this.nav.innerHTML = `<h1 id="header">PARTS</h1><p class=tagline>${Client.Explorer.items.length} item${Client.Explorer.items.length === 1 ? "" : "s"}</p>`
   ...
   this.itemNodes = {}
   for (let i = 0; i < Client.Explorer.items.length; i++) {
    const filename = Client.Explorer.items[i],
     itemNode = fileList.appendChild(document.createElement("li")),
     I = BigInt(i),
     replaceWithThis = () => (Client.here = this.selectionOffset + this.maskCardinality * I)
     ...
     if (Client.contextKeysDown) {
      if (isSelected) {
       ...
       if (this.index === 0n) replaceWithThis()
        ...
        Client.here = this.selectionOffset + newAnchorMask + newMaskIndex
       }
      }
   ...
   this.anchorNode = this.itemNodes[Client.Explorer.items[0]]
   this.anchorNode.setAttribute("data-anchor", "true")
   super.enter()
   //console.groupEnd()
  }
 ...
 static Engine = class Engine extends Parts.Composite {
  constructor() {
   super("engine", [new Parts.Bitmask("byte", 8)])
  }
  enter(byte) {
   //console.group("Entering", this)
   super.enter()
 ...
 constructor() {
  super("client", ["empty", new Client.Explorer(Client.Explorer.items), new Client.Engine()])
 }
 async enter() {
  //console.group("Entering", this)
  super.enter()
...
class Core extends Parts.Decision {
 constructor() {
  super("core", ["bootstrap", new Server(), new Client()])
