(C = {
 ...
 "scroll-y.number": { get() { return this[".node"].scrollTop } },
 ...
 "ownNode.fn": {
  get() {
   return incomingNode => {
    ...
    const search = Object.entries(this["workingTree.commit"]).map(e => e.join('=')).join('&')

    if (!search)
     node.removeAttribute('data-search')
    else
     node.setAttribute('data-search', search)

    let timeout

    if (this["tabindex.number"])
     node.tabIndex = this["tabindex.number"]
   ...
   return commit => {
    ...
    for (const name in commit) {
     ...
     if (href.startsWith('data:')) {
      ...
      description[name] = { get() { return body }, configurable: true }
      continue
     }
