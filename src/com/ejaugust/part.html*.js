return scroller.wrap(
 "<header>" + (
  `<a id=notebook-title title="⌂ Home" href=${home.canonicalURL} onclick=self._?.noop(event) onpointerdown=self._?.go(event,this)>` + (
   `<h1>` + (
 ...
 "<footer>" + (
  "<section id=quick-links>" + (
   `<a id=footer-home href=${home.canonicalURL} onclick=self._?.noop(event) onpointerdown=self._?.go(event,this)><span class=unicode-icon>⌂</span> Home</a>` +
   `<a id=footer-top href=${ejaugust.canonicalURL} onclick=self._?.noop(event) onpointerdown=self._?.go(event,this)><span class=unicode-icon>↑</span> Back to Top</a>`
  ) +
  "</section>" +
