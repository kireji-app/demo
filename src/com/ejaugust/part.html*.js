return scroller.wrap(
 "<header>" + (
  `<a id=notebook-title title="⌂ Home" href=${home.canonicalURL} onpointerdown=self._?.point(event,this)>` + (
   `<h1>` + (
 ...
 "<footer>" + (
  "<section id=quick-links>" + (
   `<a id=footer-home href=${home.canonicalURL} onpointerdown=self._?.point(event,this)><span class=unicode-icon>⌂</span> Home</a>` +
   `<a id=footer-top href=${ejaugust.canonicalURL} onpointerdown=self._?.point(event,this)><span class=unicode-icon>↑</span> Back to Top</a>`
  ) +
  "</section>" +
