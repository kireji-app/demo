...
<p>To generate the hashing URL to your current state, we join the above hash to the domain name of a specific app (in this case <code>"www.ejaugust.com"</code>) and the semantic version number of the hash function. That gives us the URL currently in your address bar:</p>
<pre id=current-hash-platform>https://www.ejaugust.com<output id=current-encoded-pathname>${encodePathname(_.routeID)}</output></pre>
