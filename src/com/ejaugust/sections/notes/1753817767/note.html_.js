...
const lastHash = RID.toHash(lastState)
...
<p>But, when discussing string length, the meaning of the fractional part of the logarithm (sometimes called the <a ${_.pointAttr()} class=external href="https://www.etymonline.com/word/mantissa">"mantissa"</a>) becomes abstract. We can't have a URL with a fraction of a character.</p>
<p>In the Kireji Web Framework, computing this logarithm is less convenient than simply hashing <math><mrow><mi>k</mi><mo>−</mo><mn>1</mn></mrow></math> using the framework's actual URL encoder:</p>
<pre>${lastHash}</pre>
<p>We know all other data strings will be the same length or shorter than than this one. So, we simply measure this string's length. When measured this way, the entropy we get is:</p>
<pre>${lastHash.length} charms</pre>
<h2>One Small Note</h2>
<p>Unlike a conventional base-64 numeral (where <math display=inline><mn>1</mn><mo>=</mo><mn>01</mn><mo>=</mo><mn>001</mn></math> and leading zeros can be omitted) this framework employs a hash function such that two different strings never map to the same integer. The string “1” maps to one integer, “01” to another and “001” to yet another. In these scheme, the empty string (<code>""</code>) maps to <math display=inline><mn>0</mn></math> whereas <code>"0"</code> actually maps to <math display=inline><mn>64</mn></math>.</p>
<p>While the maximum hash length (in charms) provides a convenient and close approximation to the entropy <math><msub><mo>log</mo><mn>64</mn></msub><mo>(</mo><mrow><mi>k</mi><mo>−</mo><mn>1</mn></mrow><mo>)</mo></math>, note that the two differ by a small constant (typically <1 charm), depending on how fully the final hash length is utilized. For precise entropy calculations, use <math><msub><mo>log</mo><mn>64</mn></msub><mo>(</mo><mrow><mi>k</mi><mo>−</mo><mn>1</mn></mrow><mo>)</mo></math> directly.
<h2>Conclusion</h2>
