const animFrame = Math.abs(Math.trunc(_.now / thisNote.hangTime) % 10 - 5) * 2
const depthEven = animFrame === 10 ? Infinity : animFrame
const depthOdd = depthEven + 1

return /* html */`
<p>From the start, I've envisioned my web framework as one that allows me to see parts its represented as equations. These equations would represent parts as a relationship between their subparts. As it happens, all ecosystem parts extend directly or indirectly from the same base type, meaning that I don't have to typeset an equation for every single part. They can just inherit and override their prototype's markup.</p>
...
<p>With default parameters, every part provides a single variable that can represent the cardinality of its state space. When I say <code>part.mathML()</code>, I get the following string as output:</p>
<pre>${sanitizeAttr(Base.mathML(0, false))}</pre>
<p>Your browser renders that string like this:</p>
<p class="math">${Base.mathML(0, false)}</p>
...
<p class="math">${EJAugustNotes.mathML(0, false)}</p>
...
<p class="math">${EJAugustSections.mathML(0, false)}</p>
...
<p>Let's try and kick up the <code>DEPTH</code> parameter a little bit. Here's that same <code>notes</code> part given four different depths:</p>
<h4><code>DEPTH = 0</code></h4>
<p class="math">${EJAugustNotes.mathML(0)}</p>
<h4><code>DEPTH = 1</code></h4>
<p class="math">${EJAugustNotes.mathML(1)}</p>
<h4><code>DEPTH = 2</code></h4>
<p class="math">${EJAugustNotes.mathML(2)}</p>
<h4><code>DEPTH = 3</code></h4>
<p class="math">${EJAugustNotes.mathML(3)}</p>
<p>Here is how <code>sections.mathML(...)</code> looks at four different depths:</p>
<h4><code>DEPTH = 0</code></h4>
<p class="math">${EJAugustSections.mathML(0)}</p>
<h4><code>DEPTH = 1</code></h4>
<p class="math">${EJAugustSections.mathML(1)}</p>
<h4><code>DEPTH = 2</code></h4>
<p class="math">${EJAugustSections.mathML(2)}</p>
<h4><code>DEPTH = 3</code></h4>
<p class="math">${EJAugustSections.mathML(3)}</p>
...
<p>This tells us that <code>notes</code> and <code>sections</code> are each the sum of their parts and that the overall ecosystem cardinality is the product of each top-level domain's cardinality. That makes sense because mutually exclusive state spaces (like <code>notes</code> and <code>sections</code>) are accounted for using simple addition. Their combined state space is the disjoint union of their subpart state spaces. The apps in the ecosystem each have a domain name and an independent state. Independent variables are accounted for using multiplication. The ecosystem's state space is therefore the cartesian product of its individual app state spaces. The apps are grouped by their top-level domain name, so each top-level domain is also a cartesian product space.</p>
...
<p class="math">${Base.mathML(0, "variable")}</p>
...
<p class="math">${EJAugustSections.mathML(0, "variable")}</p>
<h5><code>DEPTH = 1</code></h5>
<p class="math">${EJAugustSections.mathML(1, "variable")}</p>
<h5><code>DEPTH = 2</code></h5>
<p class="math">${EJAugustSections.mathML(2, "variable")}</p>
...
<p>The <code>"value"</code> type flips the equation a bit. It takes our <code>DEPTH</code>-controlled expression and makes it the left-hand side of an equation whose right-hand side is the actual cardinality of the part's state space as an integer. This integer will be in scientific notation whenever <math><msub><mi>log</mi><mn>10</mn></msub><mo>(</mo>${Base.mathML()}<mo>)</mo><mo>></mo><mn>15</mn></math>. So if we run <code>sections.mathML(depth, "value")</code>, we'll see:</p>
<h5><code>DEPTH = 0</code></h5>
<p class="math">${EJAugustSections.mathML(0, "value")}</p>
<h5><code>DEPTH = 1</code></h5>
<p class="math">${EJAugustSections.mathML(1, "value")}</p>
<h5><code>DEPTH = 2</code></h5>
<p class="math">${EJAugustSections.mathML(2, "value")}</p>
...
<h5><code>DEPTH = 0</code></h5>
<p class="math">${EJAugustSections.mathML(0, "both")}</p>
<h5><code>DEPTH = 1</code></h5>
<p class="math">${EJAugustSections.mathML(1, "both")}</p>
<h5><code>DEPTH = 2</code></h5>
<p class="math">${EJAugustSections.mathML(2, "both")}</p>
...
<p class="math">${EJAugustSections.mathML(2, "none", true)}</p>
<h4><code>Desktop.mathML(4, "value", true)</code></h4>
<p class="math">${Desktop.mathML(4, "value", true)}</p>
<h4><code>KirejiEditor.mathML(4, "value", true)</code></h4>
<p class="math">${KirejiEditor.mathML(4, "value", true)}</p>
...
<button id=anim-button ${thisNote.pointAttr()} data-playing=${thisNote.isPlaying}>
 ${thisNote["confetti.html"]}
 <b>Even Depths</b>
