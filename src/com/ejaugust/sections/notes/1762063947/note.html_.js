...
<p>My notebook is part of a larger ecosystem called the <b>Kireji Demo Ecosystem</b>. The cardinality of all of these demo apps together is about ${scientific(_.cardinality, true)}. As an integer, it looks like this:</p>
...
<pre id=current-state-ecosystem>${_.rid}</pre>
<p>Here's how it looks as a hash in base 64:</p>
<pre id=current-hash-ecosystem>${RID.toHash(_.rid)}</pre>
<p>State hashing lays the foundation for the Kireji Web Framework's routing function which provides a permanent URL for every valid state in a given ecosystem.</p>
...
<pre id=current-hash-ecosystem>https://ejaugust.com<output id=current-encoded-pathname>${RID.toPath(_.rid)}</output></pre>
