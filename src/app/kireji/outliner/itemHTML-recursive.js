...
for (let i = 0; i < subdomains.length; i++) {
 ...
 const partIndex = allParts.indexOf(subpart)
 ...
 const label = `<span class="label${subpart.isAbstract ? " abstract" : ""}">${subpart === _ ? "ecosystem" : subpart.key}</span>`
 const summary = `<summary data-index="${partIndex}"${selectedPart === subpart ? " data-selected" : ""}${IS_LAST_OF_TYPE && !hasSubparts ? ' id="lastOutlinerItem"' : ""} onclick="${propertyViewer.runtimeReference}.open(event,${partIndex})">${Array(DEPTH + +!hasSubparts).fill('<outliner-spacer></outliner-spacer>').join("")}${handle}${symbol}${label}</summary>`
