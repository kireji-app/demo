...
for (let i = 0; i < subdomains.length; i++) {
 ...
 const handle = hasSubparts ? `<svg onclick="self._?.noop(event); self._?.app.kireji.outliner.toggle(this, ${folderIndex})" xmlns="http://www.w3.org/2000/svg" viewBox="-1 -1 2 2" class="explore-toggle"><line x1="-0.41" y1="0" x2="0.41" y2="0" stroke-width="0.2" stroke-linecap="round" />${isOpen ? "" : `<line x1="0" y1="-0.41" x2="0" y2="0.41" stroke-width="0.2" stroke-linecap="round" />`}` : ""
 const label = `<span class="label${subpart.isAbstract ? " abstract" : ""}">${subpart === _ ? subpart.title : subpart.key}</span>`
 const summary = `<summary data-index="${instanceIndex}" ${selected === subpart ? "data-selected " : ""}${IS_LAST_OF_TYPE && !hasSubparts ? 'id="lastOutlinerItem" ' : ""}onclick="self._?.app.kireji.editor.selected.go(event, ${instanceIndex})">${Array(DEPTH + +!hasSubparts).fill('<outliner-spacer></outliner-spacer>').join("")}${handle}${symbol}${label}</summary>`
