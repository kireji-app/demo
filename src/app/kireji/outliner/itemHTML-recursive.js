...
for (let i = 0; i < subdomains.length; i++) {
 ...
 const summary = `<summary data-index="${partIndex}" ${selectedPart === subpart ? "data-selected " : ""}${IS_LAST_OF_TYPE && !hasSubparts ? 'id="lastOutlinerItem" ' : ""}onclick="${selected.runtimeReference}.go(event,${partIndex})">${Array(DEPTH + +!hasSubparts).fill('<outliner-spacer></outliner-spacer>').join("")}${handle}${symbol}${label}</summary>`
