...
return (
 `<section><h1><img src="${part.placeholderImage("part.png")}"/>${part.title || part.key}</h1><p><span class=marks>Part "</span>${part.domains.join("<span class=marks>.</span>")}<span class=marks>" ${part.host === "part.core.parts" ? "is the type root and has no prototype.</span>" : `extends </span><a href="#" onclick="_.app.kireji.www.editor.selected.go(event, ${allParts.indexOf(part.prototype)})"><span class=marks>"</span>` + part.prototype.domains.join("<span class=marks>.</span>") + '<span class=marks>"</span></a>'}<p><p id=description>${part.description ?? "<span class=marks>This part has no description.</span>"}</p></section>` +
 "<section>" + (
