declare interface IWorker
 ...
 /** HTML defining a `<script>` element which either bootstraps the service worker registration (if HTML is rendered from the server environment) or fetches the framework script (if HTML is rendered from the worker environment). */
