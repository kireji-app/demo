if (hydrated) {
 ...
 for (const alreadySelectedIcon of document.querySelectorAll(`wallpaper- desktop-icon[data-selected]`)) {
 ...
 for (const iconToSelect of selectedIcons) {
  const iconElement = Q(`wallpaper- desktop-icon[data-index="${desktopIcons.superset.indexOf(iconToSelect)}"]`)
