return partOutliner.getChildren(SUBJECT).map((childPart, i, childArray) => {
 ...
 const summary = `<summary data-index="${partIndex}"${partOutliner.isActive(childPart) ? " data-active" : ""}${IS_LAST_OF_TYPE && !hasSubparts ? ' id="lastOutlinerItem"' : ""} onpointerdown="${partOutliner.runtimeReference}.point(event,this,${partIndex})">${Array(DEPTH + +!hasSubparts).fill('<outliner-spacer></outliner-spacer>').join("")}${handle}${symbol}${label}</summary>`
