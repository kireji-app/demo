return partOutliner.getChildren(SUBJECT).map((childPart, i, childArray) => {
 ...
 const handle = hasSubparts ? `<svg onpointerdown="${partOutliner.runtimeReference}.togglePoint(event,this,${folderIndex})" xmlns="http://www.w3.org/2000/svg" viewBox="-1 -1 2 2" class="explore-toggle"><line x1="-0.41" y1="0" x2="0.41" y2="0" stroke-width="0.2" stroke-linecap="round" />${isOpen ? "" : `<line x1="0" y1="-0.41" x2="0" y2="0.41" stroke-width="0.2" stroke-linecap="round" />`}</svg>` : ""
 const label = `<span class="label${childPart.isAbstract ? " abstract" : ""}">${partOutliner.getLabel(childPart)}</span>`
 const summary = `<summary data-index="${partIndex}"${partOutliner.isSelected(childPart) ? " data-selected" : ""}${IS_LAST_OF_TYPE && !hasSubparts ? ' id="lastOutlinerItem"' : ""} onpointerdown="${partOutliner.runtimeReference}.point(event,this,${partIndex})">${Array(DEPTH + +!hasSubparts).fill('<outliner-spacer></outliner-spacer>').join("")}${handle}${symbol}${label}</summary>`
