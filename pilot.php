<? $Δ = [
 ...
 "https://core.parts/core-part/bootstrap.js" => '
  () => {
   ...
   globalThis.nodePool = { }
   const eventConnectors = { onclick: 0, oncontextmenu: 0, onpointerdown: 0, onpointerup: 0, onpointermove: 0, ondblclick: 0, onfocus: 0, layout: 1, manifest: 1, ondragstart: -1 };
   Object.defineProperties(HTMLElement.prototype, {
    ...
    url: {
     ...
     set(v) {
      ...
      for (const connectors in eventConnectors) {
       const type = eventConnectors[connectors];
       if (type === -1) {
        this[connectors] = e => e.preventDefault()
       } else if (proxy_keys.includes(connectors)) {
       const subproxy = this.proxy[connectors]
       switch(type) {
        case 0:
         this[connectors] = subproxy
         break;
        case 1:
         const primitive = subproxy.toPrimitive()
         this[connectors] = primitive
         const url = subproxy.headerOf().href;
         if (!(url in causality)) causality[url] = {}
         if (!(connectors in causality[url])) causality[url][connectors] = new Set()
         causality[url][connectors].add(this)
         break;
        }
       }
      }
   ...
   if (url.includes("&")) {
    ...
    query.split("&").forEach(subquery => {
     const
      url = base + "?" + subquery,
      proxy = Ω[url],
      { value, connectors, target } = proxy.headerOf().groups;
     Ω[target][connectors] = value
    })
    const response = new Response(new Int8Array([1]))
    return direct ? response : event.respondWith(response)
   }
   const proxy = Ω[url], { binary, type, value, connectors, target } = proxy.headerOf().groups;
   let string = "";
   if (value) {
    throw "deprecated"
    Ω[target][connectors] = value
 ...
 "https://core.parts/proxy/alpha.js" => '
  ({
   get:
    (_, υ) => {
     const
      regex = /^(?<protocol>[a-z+]+:\/\/?)(?:(?<host>[^\/]+?)(?:\/(?<path>(?:[^\s.?\/]+?\/)*)(?:(?<part>[a-z][a-z0-9-]*)\/?|(?<filename>[^\s?\/]*)\.(?<extension>(?<binary>png|ico|woff2|wasm)|[^\s.?\/]+))|\/(?<index>(?:[^\s.?\/]+?\/)*))(?:\?(?<connectors>[a-zA-Z][a-zA-Z0-9_]*)(?:=(?<value>-?[\d]*\.?[\d]*)(?<rest_connectors>&(?:[a-zA-Z][a-zA-Z0-9_]*=-?[\d]*\.?[\d]*)+)?$)?)?)?$/,
     ...
     const
      ...
      true_extension = Ψ.value ? "js"
                       ...
                       : (Ψ.connectors === undefined) ? Ψ.extension
                       : "uri";
     Object.defineProperties(Ψ, extras)
     if (Ψ.value)
      Ψ.target = υ.slice(0, - Ψ.connectors.length - (2 + Ψ.value.length))
       ...
       for (const url in data)
        if (url in causality)
         Object.entries(causality[url]).forEach(
          ([connectors, nodeset]) => {
           nodeset.forEach(node => node[connectors] = data[url])
          }
         )
      }
     Δ[υ] = δ
   ...
   return Object.keys(Δ).reduce((o, url) => {
    ...
    if (rootIndex !== -1) {
     const root = roots[rootIndex],
      connectors = url.slice(root.length + 1)
     const item = { url, root, connectors, rootIndex, href: Δ[url] }
     const result = ƒ(item)
 ...
 "https://core.parts/proxy/beta/set.js" => '
  (_, connectors, value) => {
   console.warn("try to use the other method directly", { connectors, value, υ })
   return Ω[Ω[Ω[υ].query(l => l.connectors === connectors ? l.url : undefined)[0]]] = value
  }',
 "https://core.parts/proxy/beta/toPrimitive.js" => '
  (hint, caller) => {
   ...
   if (imagine || primitive === undefined) {
    const proxy = Ω[υ], constructor = proxy.constructor?.toPrimitive(), Connectors = new Map(), roots = β.rootsOf()
    ...
    for (const url in Δ) {
     ...
     do {
      if (r === base) {
       if (!Connectors.has(π) || Connectors.get(π)[0] > rank) {
        Connectors.set(π, [rank, `"${Δ[url]}": ${π}`]);
       }
       break
      }
    ...
    const runtime = eval("({ \n " + [...Connectors.values()].map(x=>x[1]).join(\',\n \') + "\n}) => {\n " + constructor + "\n}");
    primitive = runtime(Ω);
 ...
 "https://pilot.parts/icons/connectors-icon/layout.css" => '
  :host {
   --size: 16px;
   width: var(--size);
   height: var(--size);
 ...
 "https://pilot.parts/icons/connectors-icon/?layout" => 'https://pilot.parts/icons/connectors-icon/layout.css',
...
Locate */
 ...
 "https://pilot.parts/programs/locate/window/explorer-view/files/manifest.uri?show_connectors" => 'https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt',
 ...
 "https://pilot.parts/programs/locate/window/explorer-view/files/manifest.uri.c.js" => '
  ...
  let connectors_count = 0, folder_count = 0, file_count = 0;
  for (const [url, name, i] of filenames) {
   if (name.includes("?") && ("" + show_connectors === "0")) continue;
   const
    proxy = Ω[url],
    groups = proxy.headerOf().groups,
    row_type = url.match(/\?[\w\d_$]+$/)
    ? (connectors_count++, "connector") : url.endsWith("/")
  ...
  Ω["https://pilot.parts/programs/locate/window/status/connectors_count.txt"] = connectors_count
  file_list.sort((a, b) => {
 ...
 "https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt" => '0',
 "https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt?fx" => 'https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt.fx.uri',
 "https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt.fx.uri" => 'https://pilot.parts/programs/locate/window/explorer-view/files/manifest.uri https://pilot.parts/programs/locate/window/status/layout.css https://pilot.parts/programs/locate/window/tool-bar/manifest.uri https://pilot.parts/programs/locate/window/tool-bar/toggle-connectors/layout.css',
 ...
 "https://pilot.parts/programs/locate/window/tool-bar/manifest.uri?show_connectors" => 'https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt',
 ...
 "https://pilot.parts/programs/locate/window/tool-bar/manifest.uri.c.js" => '
  ...
  return [[
   ...
   common_url + "toggle-connectors/",
   common_css + `:host::before { content: \'🔗\' } :host::after { content: \'${("" + show_connectors) === "0" ? "Show" : "Hide"} Connectors\' }`,
   "",
   `() => { Ω["https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt"] = (Ω["https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt"].toPrimitive() === "1") ? "0" : "1" }`,
 ...
 "https://pilot.parts/programs/locate/window/status/layout.css?connectors_count" => 'https://pilot.parts/programs/locate/window/status/connectors_count.txt',
 "https://pilot.parts/programs/locate/window/status/layout.css?show_connectors" => 'https://pilot.parts/programs/locate/window/explorer-view/show_connectors.txt',
 ...
 "https://pilot.parts/programs/locate/window/status/layout.css.c.js" => '
   const
    ...
    do_connectors = ("" + show_connectors) === "1",
    num_connectors = do_connectors ? parseInt("" + connectors_count) : undefined,
    has_connectors = do_connectors ? !!num_connectors : undefined,
    status_items = [];
   ...
   if (has_connectors) status_items.push(`${connectors_count} connector${num_connectors === 1 ? "" : "s"}`)
   return `
    :host {
     padding: 0 3px;
     height: 17px;
 ...
 "https://pilot.parts/programs/locate/window/status/connectors_count.txt" => '0',
 "https://pilot.parts/programs/locate/window/status/connectors_count.txt?fx" => 'https://pilot.parts/programs/locate/window/status/fx.uri',/*
Relate */
 ...
 "https://pilot.parts/programs/relate/window/graph/manifest.uri.c.js" => '
   const
    ...
    connectors_urls = new Set(),
    connectors_node_urls = new Set(),
    own_url = "" + address,
    graph_url = "https://pilot.parts/programs/relate/window/graph/";
   for (const url in Δ) {
    if (!url.match(/^[^?]*\?\w*$/)) continue
    if (!connectors_urls.has(url) && own_url === url.split("?")[0]) connectors_urls.add(Δ[url])
   }
   let own_node_url;
   const result_urls = [own_url /*, ...connectors_urls*/].map((address, index) => {
    const
     node_url = `${graph_url}${hash(own_url + " " + address + " node")}/node/`,
     transform_url = node_url + "transform/",
     position_url = `${node_url}position.json`;
    if (index) connectors_node_urls.add(node_url)
    ...
    return node_url
   })/*.concat([...connectors_node_urls].map(node_url=>{
 ...
 "https://pilot.parts/programs/relate/window/core-node/manifest.c.js" => '
   ...
   // Ω[]
   return [title_url, "https://pilot.parts/horizontal-line/", ...[...keys].map(connectors => {
    const
     cell_url = node_url + hash(`${href}?${connectors}`) + "/connectors/",
     label_url = cell_url + "label/";
    cell(label_url, `
     ${label_css(connectors)}
     ${common_label}
     :host {
      margin-left: 0;
      box-sizing: border-box;
...
Welcome */
 ...
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-1.txt" => 'Refreshing the page is the same as rebooting pilot.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-1.txt" => '🔗 Connectors are files that describe other files.',
 ...
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-4.txt" => '📁 Locate is the complete site map for https://pilot.parts.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-5.txt" => 'Each tab is a separate client with its own programs and files.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-6.txt" => 'There is always a virtual computer running in the background called the server.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-7.txt" => 'Loading the last save just refreshes the page.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-8.txt" => 'To discard a client without saving, simply close the tab it is running in.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-9.txt" => 'When you perform a factory reset, it completely erases everything you\'ve done and re-downloads Pilot from the server.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-10.txt" => '🔗 Connectors are used to link files to constructors.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-11.txt" => 'In 📁 Locate, right click any file to download it.',
 "https://pilot.parts/programs/welcome/window/panel/tip/tip-12.txt" => 'A \'constructor\' is a file used to generate another file.',
 "https://pilot.parts/programs/welcome/window/panel/tip/count.txt" => '13',
 ...
 "https://pilot.parts/programs/welcome/window/panel/tip/index.txt" => '2',
