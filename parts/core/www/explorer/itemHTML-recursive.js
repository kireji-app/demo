...
for (const subpart of SUBJECT) {
 const typeIcon = `<img class="type-icon" src="${subpart.super.render({ request: "theme.png", format: "datauri" })}"/>`
 const symbol = `<span><img src=${subpart.render({ request: "theme.png", format: "datauri" })}>${typeIcon}</span>`
 ...
 const label = `<span class=label>${subpart.key}${subpart.key === user.key ? `(${build.version})` : ""}</span>`
