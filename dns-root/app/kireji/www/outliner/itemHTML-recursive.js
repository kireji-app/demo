...
for (let i = 0; i < subparts.length; i++) {
 ...
 const instanceIndex = instances.indexOf(subpart)
 ...
 const summary = `<summary data-index="${instanceIndex}" ${selected === subpart ? "data-selected " : ""}${IS_LAST_OF_TYPE && !hasSubparts ? 'id="lastOutlinerItem" ' : ""}onclick="_.app.kireji.www.editor.go(event, ${instanceIndex})">${Array(DEPTH + +(subpart.length === 0)).fill('<outliner-spacer></outliner-spacer>').join("")}${handle}${symbol}${label}</summary>`
