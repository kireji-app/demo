...
part.bunnies = part.bunnyNames.map((bunnyName, bunnyIndex) => {
 ...
 img.setAttribute("src", `https://${framework.host}/${bunnyName}-up.gif?raw`)
 ...
 placeholder.innerHTML = `${Framework.isProduction ? "" : `<h1>Area ${area}</h1>`}<span id=float><img src="https://${framework.host}/${bunnyName}-up.gif?raw"><span class=thin>is </span><span>coming soon.</span></span>`
 return img
})
