 ...
 for (const arm of match) {
  ...
  if (ROUTE_ID < arm.cardinality) {
   ...
   if (match.arm !== arm) {
    ...
    /*
     If a previous arm exists, we need to end the task.
     match.arm?.captureTaskEnd()
     But now is not the time for tasks. It is route-only time.
     We will simply make a note of the expired task.
    */
   ...
   // routeCapture allows duplicate assignment, so we should not call it if the given arm already has this id
   // then again ... if there has been a change
   if (ROUTE_ID !== arm.routeID)
    arm.routeCapture(ROUTE_ID)
