/*
 If the part was directly set, there will be no subparts.
...
if (SUBPARTS) {
 ...
 if (SUBPARTS.length !== 1)
  ...
  ') of subparts bubbled their route to part "' + match.host + '")'
  ...
  */
  if (!match.enabled) {
   /*
    ...
    When I call routePopulate below, it will set my enabled state.
    ...
    TODO: The last remaining concern here, it seems, is that this item may need to
     be marked as waking up so that its view can be added. We will see as we get to the view flow later.
   */
  }
 }
