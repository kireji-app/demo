...
const currentHost = IS_PRODUCTION ? location.host : DEVELOPMENT_HOST
return `
<task-menu style="${menu.arm.render("style-attr")}">
 <sidebar- onclick="menu.noop(event)">
 <ul id=wallpapers>${["desktop.parts", "kireji.io", "core.parts", "ejaugust.com", "kireji.app", "orenjinari.com"].map(hostBase => {
 const kioskHost = "www." + hostBase
 const kioskPart = new Part(kioskHost)
 return `
  <li class=task-link${kioskHost === currentHost ? `data-here=true` : ""}>
   <a href="https://${kioskHost}" onclick="menu.goWallpaper(event)">
    <img src="${kioskPart.render({ request: "icon.uri", format: "datauri" })}" class=task-icon />
    <span class=label>${kioskPart.render("title")}</span>
   </a>
  </li>`
}).join("")}</ul>
  <hr>
