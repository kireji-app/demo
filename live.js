const
 protocolumns = {
  ...
  "version.number": { get() { return 49 / 1000 } },
   ...
   get() {
    return name => clickEvent => {
     if (name in (this._toggles ??= {})) {
      ...
      this._toggles[name] = Object.getPrototypeOf(hostRow)["insert.fn"](name)
     }
  ...
  "address/": { get() { return `.node=./auto.node&.html=./path.uri&.tag=data:text/tag,addressbar-&.css=./pill.css` } },
  ...
  "core.parts/": { get() { return `.node=./auto.node&.tag=./name.tag&background.color=grey.color&.children=./editor.children&.layout=theme.layout&color.html=./background.color&sidebar-open.bool=true.bool` } },
  ...
  "main-layout/": { get() { return `layout.css=main-layout.css` } },
  ...
  "theme.layout": { get() { return ['main-layout'] } },
  ...
  "portfolio.css": {
   get() {
    return `:host {
     ...
     align-items: stretch;
     background: url(data:image/png;base64,${this["blue-grid.png"]});
 ...
 hostRow = protorow["branch.fn"](location.host.slice(location.host.startsWith('dev.') ? 4 : 0), { ".host": { value: location.host }, ".node": { get() { return this["auto.node"] } } })

onload = () => {
 hostRow["ownNode.fn"](document.body)
 protorow["signal.fn"]()
 // protorow["test.fn"]()
}